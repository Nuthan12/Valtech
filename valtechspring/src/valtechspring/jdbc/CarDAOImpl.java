...
public class CarDAOImpl implements CarDAO {
	...
	public void save(Car car) {
		...
		temp.update(
				"insert into car (kms,numOfOwners,validInsurance,color,make,model,price,yearManufactured,version,id) "
						+ " values(?,?,?,?,?,?,?,?,?,?)",
				car.getCarDetails().getKms(), car.getCarDetails().getNumberOfOwners(),
				car.getCarDetails().isValidInsurance(), car.getColour(), car.getMake(), car.getModel(),
				car.getVersion(), car.getYearManufactured(), car.getPrice(), car.getId());
	}
	...
	public void update(Car car) {
		JdbcTemplate temp = new JdbcTemplate(dataSource);
		temp.update(
				"update car set kms = ?,numOfOwners = ? , validInsurance = ? ,color = ? "
